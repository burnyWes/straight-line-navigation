...
1. **Gather metadata**:
   ...
   - **Assign a sequential story number** `COE-xxx`:
     - Scan the `story:` frontmatter field of every existing plan in `docs/agents/plans/` and take the highest `COE-NNN`.
     - The new plan's number is that maximum plus one. If no plan has a story number yet, start at `COE-001`.
     - Zero-pad to three digits (`COE-001`, `COE-042`, …).
     - Record it in the plan's `story:` frontmatter field and prefix the `# PLAN:` title with it (e.g. `# PLAN: COE-007 — <Title>`). The commit-skill reads this `story:` field to prefix commit messages.
